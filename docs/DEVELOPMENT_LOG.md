...
## 2026-08-12 — 人工工作簿自动化衔接

在等待 A/B 人工复核期间完成不依赖真值的工程工作：实现易填版 XLSX 的只读导入、内部映射防篡改、词汇/证据/完整性检查、双人身份检查、授权比较和裁决队列。修复了 A/B 对纳入/排除意见不一致时被旧逻辑直接排除的问题；现在所有决定性分歧都必须由第三人裁决。另增加风险盲标包的失败关闭审计，禁止来源标签、预期结果和授权占位符进入正式标注上下文。

本地验收包括 32/32 单元测试、ruff、示例 Schema 验证，以及基于实际易填版 A/B 工作簿的端到端导入冒烟测试。冒烟测试成功识别两名不同复核员、5 个场景组、5/5 一致排除，并生成空授权冻结文件；该数据仅验证流程，不是人工研究结论。

## 2026-08-12 — P3 选择性治理工程骨架

在不等待人工真值的前提下继续完成模型与决策基础设施，但不把 synthetic 标签形成的指标
写作论文证据：

1. 实现四个累积二分类器组成的有序 Logistic 风险模型，推理时执行单调投影，输出合法的
   L0～L4 概率分布；
2. 将原验证场景组预先分配为互斥的概率校准组和 Conformal 校准组，测试组只做最后评估；
3. 实现温度缩放和有限样本 split-conformal 分位数；
4. 实现硬规则优先决策：只有不可变硬规则可以自动 `deny`，纯统计高风险最多升级为
   `approve`，规则允许时才使用 `rewrite/isolate`；
5. 增加逐样本概率、风险集合、决策、原因码、数据角色、输入/配置哈希、Git commit、依赖
   版本和配置快照；输出目录原子发布且禁止覆盖；
6. 增加数据角色场景组泄漏、概率合法性、确定性、校准归属、非空预测集和硬拒绝边界测试。

首次冒烟正确暴露样本不足：概率校准仅2条、Conformal校准仅4条，温度优化达到搜索边界，
Conformal 风险集合退化为全部5类。程序将这些限制写入 `metrics.json`，并强制
`formal_research_use_allowed=false`。这证明安全降级和告警路径生效，不构成模型性能结论。
